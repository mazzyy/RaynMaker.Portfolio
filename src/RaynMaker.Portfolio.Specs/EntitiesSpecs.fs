...
module ``Given a sequence of events`` =
    ...
    let ``<When> last event is a 'buy' <Then> price is taken from that``() =
        [
            at 2016 10 10 |> buy "Joe Inc" 10 10.0 |> toDomainEvent
            at 2016 11 10 |> sell "Joe Inc" 5 11.0 |> toDomainEvent
            at 2016 12 10 |> buy "Joe Inc" 5 12.0 |> toDomainEvent
            at 2016 12 12 |> buy "Jacky Limited" 15 200.0 |> toDomainEvent
        ] 
    ...
    let ``<When> last event is a 'priced' <Then> price is taken from that``() =
        [
            at 2016 10 10 |> buy "Joe Inc" 10 10.0 |> toDomainEvent
            at 2016 11 10 |> sell "Joe Inc" 5 11.0 |> toDomainEvent
            at 2016 12 11 |> price "Joe Inc" 14.0 |> toDomainEvent
            at 2016 12 12 |> buy "Jacky Limited" 15 200.0 |> toDomainEvent
        ] 
        |> Events.LastPriceOf <| isin "Joe Inc"
...
module ``Given a position`` =
    ...
    let company = "Joe Inc"
    let openNew at = Positions.openNew at (isin company) company
    let buy = buy company
    let sell = sell company
    let dividends = dividends company
    ...
    let ``<When> buying new shares <Then> share count and position value increased by bought shares``() =
        ...
        p.Invested |> should equal ((5M * 17.5M<Currency> |> withFee) + (7M * 18.9M<Currency> |> withFee))
    ...
    let ``<When> selling some shares <Then> share count and position value decreased by the sold shares``() =
        ...
        p.Invested |> should equal (5M * 17.5M<Currency> |> withFee)
        p.Dividends |> should equal 0.0M<Currency>
        p.Payouts |> should equal (2M * 18.9M<Currency> |> withoutFee)
    ...
    let ``<When> selling all shares <Then> position is closed, share count is zero and position value is calculated by sold price``() =
        ...
        p.Invested |> should equal (5M * 17.5M<Currency> |> withFee)
        p.Dividends |> should equal 0.0M<Currency>
        p.Payouts |> should equal (5M * 18.9M<Currency> |> withoutFee)
...
module ``Given a list of stock events`` =
    ...
    let ``<When> multipe stocks are bought <Then> multiple positions are created``() =
        let positions =
            [
                at 2015 04 01 |> buy  "A Inc" 100 17.5  |> toDomainEvent
                at 2015 06 01 |> buy  "C Limited" 5 102.0 |> toDomainEvent
                at 2015 10 01 |> dividends "A Inc" 2.1 |> toDomainEvent
                at 2016 01 01 |> buy  "B Corp" 20 42.1 |> toDomainEvent
                at 2016 06 01 |> buy  "B Corp" 10 51.7 |> toDomainEvent
                at 2016 10 01 |> dividends "A Inc" 2.9 |> toDomainEvent
                at 2016 12 01 |> sell  "C Limited" 5 202.0 |> toDomainEvent
            ]
            |> Positions.create
        ...
        ``A Inc``.Invested |> should equal (17.5M<Currency> * 100.0M |> withFee)
        ...
        ``B Corp``.Invested |> should equal ((42.1M<Currency> * 20.0M |> withFee) + (51.7M<Currency> * 10.0M |> withFee))
        ...
        ``C Limited``.Invested |> should equal (102M<Currency> * 5.0M |> withFee)
        ``C Limited``.Payouts |> should equal (202.0M<Currency> * 5.0M |> withoutFee);
